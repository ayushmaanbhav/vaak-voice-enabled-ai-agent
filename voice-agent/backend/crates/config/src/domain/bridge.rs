...
//!
//! let config = MasterDomainConfig::load("gold_loan", "config/")?;
...
use voice_agent_core::traits::{
    ...
    // Competitors
    CompetitorAnalyzer, CompetitorInfo, CompetitorType, ConfigCompetitorAnalyzer,
    ComparisonPoint,
    // Objections
...
impl DomainBridge {
    ...
    pub fn lead_scoring(&self) -> Arc<dyn LeadScoringStrategy> {
        ...
        Arc::new(ConfigLeadScoring::new(CoreScoringConfig {
            ...
            urgency_keyword_score: scoring.urgency.keyword_score,
            ...
            // Information scores (gold_details_score -> asset_details in generic interface)
            info_contact_score: scoring.information.contact_info_score,
            info_asset_details_score: scoring.information.gold_details_score,
    ...
    pub fn competitor_analyzer(&self) -> Arc<dyn CompetitorAnalyzer> {
        let competitors: Vec<CompetitorInfo> = self.config.competitors_config.competitors
            .iter()
            .map(|(id, entry)| {
                let comp_type = match entry.competitor_type.to_lowercase().as_str() {
                    "bank" => CompetitorType::Bank,
                    "nbfc" => CompetitorType::Nbfc,
                    "informal" => CompetitorType::Informal,
                    _ => CompetitorType::Nbfc,
                };
