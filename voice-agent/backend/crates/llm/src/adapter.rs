...
use voice_agent_core::{
    ...
    Error, GenerateRequest, GenerateResponse, LanguageModel, Result, StreamChunk, ToolDefinition,
};
            ...
            .map(|m| {
                crate::prompt::Message {
                    role: match m.role {
                        ...
                        voice_agent_core::llm_types::Role::Assistant => {
                            crate::prompt::Role::Assistant
                        },
...
impl LanguageModel for LanguageModelAdapter {
    async fn generate(&self, request: GenerateRequest) -> Result<GenerateResponse> {
        ...
        self.backend
            ...
            .map_err(|e| Error::Llm(format!("generate failed (model={}): {}", model, e)))
    }
    ...
    async fn generate_with_tools(
        ...
        self.backend
            ...
            .map(|result| {
                ...
                let tool_calls: Vec<voice_agent_core::llm_types::ToolCall> =
                    crate::prompt::parse_tool_call(&result.text)
            ...
            .map_err(|e| {
                Error::Llm(format!(
                    "generate_with_tools failed (model={}, tools={}): {}",
                    model, tool_count, e
                ))
            })
    }
